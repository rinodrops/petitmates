...
**キャラクター** タブでは `[characters]`（`bearded_dragon` / `pond_turtle` / `leopard_gecko`）の起動時スポーン数を設定します。3 種とも `0` の場合はフトアゴヒゲトカゲが 1 体だけスポーンします。メニューの **追加 / 最後のキャラクターを削除** はランタイム専用で、起動時の数は変わりません（再起動で保存値に戻ります）。
