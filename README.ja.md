...
- **設定ウィンドウ** — メニューから ConfUI で `user.toml` を編集できます（アプリに同梱）。ConfUI を閉じると Petit Mates が **再起動** し、表示・発話・天気（キャラクターサイズ含む）が確実に反映されます。
- macOS: メニューバーの **設定…**。**Option** を押しながらメニューを開くと **設定ファイルを開く**。
- Windows: トレイメニューの **設定…**。**Alt** で **設定ファイルを開く**。
