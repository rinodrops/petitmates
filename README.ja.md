...
- **設定…** — 設定ウィンドウ（ConfUI）で表示・発話・天気などを編集します。ConfUI を閉じると Petit Mates が **再起動** し、設定が反映されます。
...
キャラクターサイズ，吹き出しの文字サイズ，吹き出し言語（`os` / `en` / `ja`），発話のオン/オフと間隔，天気（都市名）を変更できます。**設定ウィンドウを閉じるとアプリが再起動** し、設定が読み込まれます。

テキストエディタで直接編集した場合（Option/Alt + メニュー）は，反映のためにアプリを手動で再起動してください。
...
**設定**
- **設定ウィンドウ** — メニューバー（macOS）またはタスクトレイ（Windows）の **設定…** から、同梱の ConfUI で `user.toml` を編集できます。キャラクターサイズ，吹き出しの文字サイズ，吹き出し言語（`os` / `en` / `ja`），発話のオン/オフと間隔，天気（都市名）を変更できます。
- **閉じると反映** — ConfUI を閉じると Petit Mates が **再起動** し、起動時に設定が読み込まれます（スプライトのスケールや天気を含む）。ランタイムホットリロードは試行後に見送り、この方式に変更しました。
...
- ConfUI が浮動小数で保存した値（例: `300.0`）を、`sprite_size` や `font_size` などの整数フィールドのパースでも受け付けるようになりました。
